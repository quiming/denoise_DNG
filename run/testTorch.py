...
def denoise_raw(input_path, output_path,ground_path ,  model_path, black_level, white_level):
    ...
    pre-process
    """
    raw_data_expand_c, height, width = read_image(input_path) #raw_data_expand_c: (1736, 2312, 4)
    #print('raw:',raw_data_expand_c.shape)
    #print('raw_data_expand_c:',raw_data_expand_c.shape)
    raw_data_expand_c_normal = normalization(raw_data_expand_c, black_level, white_level)  #(1736, 2312, 4)
    #print(raw_data_expand_c_normal.shape)
    raw_data_expand_c_normal = torch.from_numpy(np.transpose(
        raw_data_expand_c_normal.reshape(-1, height//2, width//2, 4), (0, 3, 1, 2))).float()   #raw_data_expand_c_normal: torch.Size([1, 4, 1736, 2312])
    #print('raw_data_expand_c_normal:', raw_data_expand_c_normal.shape)
    net = Unet()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str, default="model_train_4x100_50.pth")
    parser.add_argument('--black_level', type=int, default=1024)
    parser.add_argument('--white_level', type=int, default=16383)
    parser.add_argument('--input_path', type=str, default="dataset/noisy/5_noise.dng")
    parser.add_argument('--output_path', type=str, default="outcome/out.dng")
    parser.add_argument('--ground_path', type=str, default="dataset/ground_truth/5_gt.dng")
