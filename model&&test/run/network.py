...
class Unet(nn.Module):
    def __init__(self, in_channels=4, out_channels=4):
        ...
        self.conv2_t = nn.Conv2d(64, 32, kernel_size=1)
        ...
        self.conv5 = Inception(256,128,(128,256),(32,64),64)
        self.Drop = nn.Dropout(0.5)

        # self.conv5_n = Inception(512, 256, (256, 512), (64, 128), 128)
        # self.conv5_n1 = nn.Conv2d(1024,512,kernel_size=3,stride=1,padding=1)
        # self.conv5_nt = nn.Conv2d(512,256,kernel_size=1)
        # self.conv5_n2 = nn.Conv2d(256,512,kernel_size=3,stride=1,padding=1)

        self.upv6 = nn.ConvTranspose2d(512, 256, 2, stride=2)
        self.conv6_1 = nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1)
        self.conv6_t = nn.Conv2d(256, 125, kernel_size=1)
        self.conv6_2 = nn.Conv2d(125, 256, kernel_size=3, stride=1, padding=1)
        self.Drop = nn.Dropout(0.5)

        self.upv7 = nn.ConvTranspose2d(256, 128, 2, stride=2)
        self.conv7_1 = nn.Conv2d(256, 128, kernel_size=3, stride=1, padding=1)
        self.conv7_t = nn.Conv2d(128, 64, kernel_size=1)
        self.conv7_2 = nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1)
        self.Drop = nn.Dropout(0.5)

        self.upv8 = nn.ConvTranspose2d(128, 64, 2, stride=2)
        self.conv8_1 = nn.Conv2d(128, 64, kernel_size=3, stride=1, padding=1)
        self.conv8_t = nn.Conv2d(64, 32, kernel_size=1)
        self.conv8_2 = nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        conv2 = self.leaky_relu(self.conv2_t(conv2))
        ...
        conv5 = self.leaky_relu(self.conv5(pool4))
        conv5 = self.Drop(conv5)


        # conv5_n = self.leaky_relu(self.conv5_n(conv5))
        # conv5_n = self.leaky_relu(self.conv5_n1(conv5_n))
        # conv5_n = self.leaky_relu(self.conv5_nt(conv5_n))
        # conv5_n = self.leaky_relu(self.conv5_n2(conv5_n))
        # conv5 = self.Drop(conv5_n)
        # #print(conv5.shape)

        up6 = self.upv6(conv5)
        up6 = torch.cat([up6, conv4], 1)
        conv6 = self.leaky_relu(self.conv6_1(up6))
        conv6 = self.leaky_relu(self.conv6_t(conv6))
        ...
        conv7 = self.leaky_relu(self.conv7_t(conv7))
        ...
        conv8 = self.leaky_relu(self.conv8_t(conv8))
...
if __name__ == "__main__":
    ...
    #              Total number of paramerters in networks is 3325161
